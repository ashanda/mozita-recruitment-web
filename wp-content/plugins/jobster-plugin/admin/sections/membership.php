...
if (!function_exists('jobster_admin_membership')): 
    function jobster_admin_membership() {
        ...
        add_settings_field(
            'jobster_payment_currency_field',
            __('Payment Currency', 'jobster'),
            'jobster_payment_currency_field_render',
            'jobster_membership_settings',
            'jobster_membership_section',
            array(
                'class' => 'pxp-membership-settings-field-all pxp-membership-settings-field-currency'
            )
        );
        add_settings_field(
...
if (!function_exists('jobster_payment_currency_field_render')): 
    function jobster_payment_currency_field_render() { 
        $options = get_option('jobster_membership_settings');

        $currencies = array('USD','EUR','AUD','BRL','CAD','CZK','DKK','HKD','HUF','ILS','INR','JPY','MYR','MXN','NOK','NZD','PHP','PLN','GBP','SGD','SEK','CHF','TWD','THB','TRY'); ?>

        <select
            name="jobster_membership_settings[jobster_payment_currency_field]" 
            id="jobster_membership_settings[jobster_payment_currency_field]"
        >
            <?php foreach ($currencies as $currency) { ?>
                <option 
                    value="<?php echo esc_attr($currency); ?>" 
                    <?php selected(
                        isset($options['jobster_payment_currency_field'])
                        && $options['jobster_payment_currency_field'] == $currency
                    ) ?>
                >
